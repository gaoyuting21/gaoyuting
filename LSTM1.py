# -*- coding: utf-8 -*-
"""
Created on Tue Oct 29 17:11:56 2019
...
#from tensorflow.compat.v2.keras.layers.ConvLSTM2D import ConvRNN2D
...
class ConvLSTM2DCell(DropoutRNNCellMixin, Layer):
  ...
  def build(self, input_shape):
    ...
    kernel_shape = self.kernel_size + (input_dim-1, self.filters * 4)
  ...
  def call(self, inputs, states, training=None):
    ...
    #x_f_c = self.input_conv_u(inputs_f, cov_kernel_f,  padding=self.padding)
    
    #x_c_c = self.input_conv_u(inputs_c, cov_kernel_c,  padding=self.padding)
    
    #x_o_c = self.input_conv_u(inputs_o, cov_kernel_o,  padding=self.padding)    
    ############################################################
    ...
    i = self.recurrent_activation(x_i_c + h_i)
  ...
  def input_conv_u(self, x, w, b=None, padding='valid'):
    conv_out = K.conv2d(x, w, strides=self.strides,
                        padding=padding,
