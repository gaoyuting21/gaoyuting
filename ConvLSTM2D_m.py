...
class ConvLSTM2DCell_2(DropoutRNNCellMixin, Layer):
  ...
  def __init__(self,
    ...
    #############################
    self.extra_cov_number = extra_cov_number
    self.cov_kernel_size = cov_kernel_size
    self.kernel_size_1 = conv_utils.normalize_tuple(cov_kernel_size, 2, 'kernel_size_1')
    ##################################
  ...
  def build(self, input_shape):
    ...
    cov_kernel_shape = self.kernel_size_1+(self.filters, self.filters * 4 * extra_cov_number)
    ...
    #############################################
    recurrent_kernel_shape_c = self.kernel_size_1 + (self.filters, self.filters * 5* extra_cov_number)
  ...
  def call(self, inputs, states, training=None):
    ...
    cov_kernel_g = array_ops.split(self.cov_kernel, extra_cov_number, axis=3)
        
    if extra_cov_number > 1:
        for temp in range(1,extra_cov_number):
            cov_kernel_i=[]
            cov_kernel_f=[]
            cov_kernel_c=[]
            cov_kernel_o=[]
        
            (cov_kernel_i_temp, cov_kernel_f_temp,
             cov_kernel_c_temp, cov_kernel_o_temp) = array_ops.split(self.cov_kernel_g[temp], 4, axis=3)
        
            cov_kernel_i.append(cov_kernel_i_temp)
            cov_kernel_f.append(cov_kernel_f_temp)
            cov_kernel_c.append(cov_kernel_c_temp)
            cov_kernel_o.append(cov_kernel_o_temp)
    else:
        (cov_kernel_i,cov_kernel_f,
         cov_kernel_c,cov_kernel_o) = array_ops.split(self.cov_kernel_g, 4, axis=3)
    ...
    else if extra_cov_number > 1:
        x_i_c = self.input_conv_u(x_i, cov_kernel_i[0],  padding=self.padding)
        
        x_f_c = self.input_conv_u(x_f, cov_kernel_f[0],  padding=self.padding)
        
        x_c_c = self.input_conv_u(x_c, cov_kernel_c[0],  padding=self.padding)
    
        x_o_c = self.input_conv_u(x_o, cov_kernel_o[0],  padding=self.padding)
        temp = 1
        for temp in range (1,extra_cov_number):
        x_i_c = self.input_conv_u(x_i_c, cov_kernel_i[temp],  padding=self.padding)
        
        x_f_c = self.input_conv_u(x_f_c, cov_kernel_f[temp],  padding=self.padding)
        
        x_c_c = self.input_conv_u(x_c_c, cov_kernel_c[temp],  padding=self.padding)
    
        x_o_c = self.input_conv_u(x_o_c, cov_kernel_o[temp],  padding=self.padding)
...
class ConvLSTM2D_2(ConvRNN2D):
  ...
  @property
  def extra_cov_number(self):
    return self.cell.extra_cov_number
